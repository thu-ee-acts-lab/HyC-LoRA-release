...
class FusedMistralLayerIntraFunc(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        x: torch.Tensor,
...
class FusedMistralLayerIntra(torch.nn.Module):
    def __init__(
        self,
    ):
        super(FusedMistralLayerIntra, self).__init__()
        self.iteration = 0
        self.static_value = {
    ...
    def forward(
        ...
        x_medium_channel_idx, x_medium_scale, \
        x_norm_2_scale, \
        gate_scale, up_scale, fn_scale, hadamard_scale = FusedMistralLayerIntraFunc.apply(
            input,
            #############attention part#############
            norm_weight_1,
            norm_bias_1,
