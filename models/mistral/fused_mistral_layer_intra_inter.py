...
class FusedMistralLayerIntraInterFunc(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        x: torch.Tensor,
...
class FusedMistralLayerIntraInter(torch.nn.Module):
    def __init__(
        self,
    ):
        super(FusedMistralLayerIntraInter, self).__init__()
        self.iteration = 0
        self.static_value = {
    ...
    def forward(
        ...
        x_medium_channel_idx, x_medium_scale, \
        x_norm_2_scale, \
        gate_main_scale, up_main_scale = FusedMistralLayerIntraInterFunc.apply(
            input,
            #############attention part#############
            norm_weight_1,
            norm_bias_1,
