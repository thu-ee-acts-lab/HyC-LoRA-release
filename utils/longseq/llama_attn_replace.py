...
import models
import models.llama_flash_attn
...
def forward_flashattn(
    ...
    #! convert the forward function
    output_unpad = self.flash_attention(
    ...
    output = output.reshape(bsz, 2, q_len, nheads // 2, self.head_dim).transpose(1, 2).reshape(bsz, q_len, nheads, self.head_dim)
...
def replace_llama_attn(use_flash_attn=True, use_full=False, inference=False):
    if use_flash_attn:
        ...
        if inference:
            models.llama_flash_attn.modeling_llama.LlamaModel._prepare_decoder_attention_mask = _prepare_decoder_attention_mask_inference
            models.llama_flash_attn.modeling_llama.LlamaAttention.forward = forward_flashattn_inference
        else:
            models.llama_flash_attn.modeling_llama.LlamaModel._prepare_decoder_attention_mask = (
            ...
            if use_full:
                print("Using full flash attention")
            else:
                print("Using partial flash attention")
    else:
        models.llama.modeling_llama.LlamaAttention.forward = forward_noflashattn
        print("Using no flash attention")
